...
public class BlogArticleRepositoryTests
{
    ...
    [Test]
    public async Task SearchAsync_ReturnsMatchingArticles_CaseInsensitive()
    {
        var articles = new[]
        {
            new BlogArticleEntity
            {
                ...
                Author = "Author 1",
                Slug = "intro-postgresql"
            },
            new BlogArticleEntity
            {
                Id = Guid.NewGuid(),
                Title = "Advanced C# Techniques",
                Description = "Master PostgreSQL queries",
                Body = "Content",
        ...
        var result = await _repository.SearchAsync("postgresql", 1, 10);

        Assert.That(result.Items.Count, Is.EqualTo(2));
        Assert.That(result.TotalCount, Is.EqualTo(2));
        Assert.That(result.Items[0].Title, Is.EqualTo("Introduction to PostgreSQL"));
        Assert.That(result.Items[1].Description, Does.Contain("PostgreSQL"));
    }
    ...
    public async Task SearchAsync_ReturnsPagedResults()
    {
        ...
        var result = await _repository.SearchAsync("test", 2, 5);
    ...
    [Test]
    public async Task SearchAsync_OrdersByDatePostedDescending()
    {
        var articles = new[]
        {
            new BlogArticleEntity
        ...
        var result = await _repository.SearchAsync("search term", 1, 10);

        Assert.That(result.Items.Count, Is.EqualTo(2));
        Assert.That(result.Items[0].Title, Is.EqualTo("Newer Article"));
        Assert.That(result.Items[1].Title, Is.EqualTo("Older Article"));
    }
}
