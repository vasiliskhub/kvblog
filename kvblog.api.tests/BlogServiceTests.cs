...
public class BlogServiceTests
{
    ...
    {
        var id = Guid.NewGuid();
        var entity = new BlogArticleEntity { Id = id, Title = "t", Body = "b", Description = "d", Slug = "slug" };
        ...
        Assert.That(result.Value?.Title, Is.EqualTo("t"));
        Assert.That(result.Value?.Body, Is.EqualTo("b"));
        Assert.That(result.Value?.Description, Is.EqualTo("d"));
    }
    ...
    public async Task GetAllArticlesAsync_ReturnsPagedResult()
    {
        ...
        Assert.That(result.Value.Items[0].Id, Is.EqualTo(id));
        Assert.That(result.Value.PageNumber, Is.EqualTo(2));
        Assert.That(result.Value.PageSize, Is.EqualTo(5));
    ...
    {
        var id = Guid.NewGuid();
        var entity = new BlogArticleEntity { Id = id, Title = "search", Body = "b", Description = "d", Slug = "slug" };
        var pagedDto = new PagedResultDto<BlogArticleEntity>
        {
            Items = new List<BlogArticleEntity> { entity },
            PageNumber = 1,
    ...
    public async Task CreateArticleAsync_CallsRepositoryWithMappedEntity()
    {
        ...
        var service = new BlogService(repo, _mapper);
        var result = await service.CreateArticleAsync(upsert);
    ...
    public async Task UpdateArticleAsync_UpdatesExistingEntity()
    {
        ...
        var service = new BlogService(repo, _mapper);
        var result = await service.UpdateArticleAsync(id, upsert);
    ...
    [Test]
    public async Task UpdateArticleAsync_ReturnsNotFoundIfEntityNotFound()
    {
        var id = Guid.NewGuid();
        var repo = Substitute.For<IBlogArticleRepository>();
    ...
    {
        var id = Guid.NewGuid();
        var existing = new BlogArticleEntity { Id = id, Title = "t", Body = "b", Description = "d", Slug = "slug" };
        var repo = Substitute.For<IBlogArticleRepository>();
        repo.GetByIdAsync(id).Returns(existing);
    ...
    [Test]
    public async Task DeleteArticleAsync_ReturnsNotFoundIfEntityNotFound()
    {
        var id = Guid.NewGuid();
        var repo = Substitute.For<IBlogArticleRepository>();
        repo.GetByIdAsync(id).Returns((BlogArticleEntity?)null);
        ...
        Assert.That(result.IsSuccess, Is.False);
        Assert.That(result.ErrorType, Is.EqualTo(ErrorType.NotFound));
        await repo.DidNotReceive().DeleteAsync(id);
    }
}
